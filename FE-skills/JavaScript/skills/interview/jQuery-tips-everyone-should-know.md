...
## [来源 Github](https://github.com/AllThingsSmitty/jquery-tips-everyone-should-know)

A collection of simple tips to help up your jQuery game.

1. [Checking If jQuery Loaded(检测jQuery是否已经加载)](#checking-if-jquery-loaded)
1. [Back to Top Button(回到顶部按钮)](#back-to-top-button)
1. [Preload Images(预加载图片)](#preload-images)
1. [Checking If Images Are Loaded(判断图片是否已经加载)](#checking-if-images-are-loaded)
1. [Fix Broken Images Automatically(自动修复破损图片)](#fix-broken-images-automatically)
1. [Toggle Classes on Hover(hover 的时候切换 class)](#toggle-classes-on-hover)
1. [Disabling Input Fields(输入框禁用)](#disabling-input-fields)
1. [Stop the Loading of Links(停止链接加载)](#stop-the-loading-of-links)
1. [Cache jQuery Selectors(缓存 jQuery 选择器)](#cache-jquery-selectors)
