...
function buildMenuDirectConnectionItem(currentProfile) {
    ...
    if (currentProfile.proxyMode == ProfileManager.ProxyModes.direct)
        item.addClass("checked"),item.find("span").addClass("blue");
...
function buildMenuOverseasModeItem(currentProfile) {
    var item = $("#overseasMode");
    item.click(onSelectProxyItem);
    var profile = ProfileManager.overseasModeProfile;
    item[0].profile = profile;
    if (currentProfile.proxyMode == ProfileManager.ProxyModes.overseas)
        item.addClass("checked"),item.find("span").addClass("blue");
}
...
function buildMenuItems() {
	...
	buildMenuOverseasModeItem(currentProfile);
...
$("#GoAgent").click(function(){
	var iconDir = "../boost/images/new/";
	localStorage.isOpen="0";
	console.log(localStorage.isOpen);
	chrome.browserAction.setTitle({ title:item.find("span").text()});
	chrome.browserAction.setIcon({ path:iconDir+item.attr("id")+".png"});
	window.open(iconDir+item.attr("id")+".png");
})
},1000)
function onSelectProxyItem() {
    ...
    if(item.attr("id")=="directConnection" || item.attr("id")=="automaticMode" || item.attr("id")=="systemProxy"){
		console.log("qita");
	    localStorage.isOpen="0";
		console.log(localStorage.isOpen);		
	}else if(item.attr("id")=="overseasMode"){
		console.log("haiwai");
	...
	$("#menu .item span").removeClass("blue");
    item.addClass("checked");
	item.find("span").addClass("blue");
